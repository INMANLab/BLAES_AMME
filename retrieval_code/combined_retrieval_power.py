...
def plot_bc_bar_graph(data, out_dir, label):
    ...
    plot_filtered_significance_bargraph(
        ...
        f'{label} Retrieval Baseline-Corrected Power Diff (Stim - No Stim)',
        'Baseline-Corrected Power Diff',
        'Bargraph_baseline_corrected_powerDiff_byROI_retrieval_filtered_sig.png',
...
def plot_bc_bar_by_memory(data, out_dir, label):
    ...
    for mem, mem_label in [('remembered', 'Remembered'), ('forgotten', 'Forgotten')]:
        ...
        plot_filtered_significance_bargraph(
            ...
            f'{label} Retrieval Baseline-Corrected Power Diff (Stim - No Stim), {mem_label} Trials',
            'Baseline-Corrected Power Diff',
            f'Bargraph_baseline_corrected_powerDiff_byROI_retrieval_{mem}_filtered_sig.png',
            caption_text=f'Method: For each patient and ROI, baseline-corrected spectra are averaged across stim and no-stim {mem_label.lower()} retrieval trials separately, band means are computed, then Stim-NoStim is taken. Bars = mean across patients, error bars = SEM, dots = patient values.',
        )
...
def plot_bc_remembered_forgotten(data, out_dir, label):
    ...
    for roi in sorted(all_rois):
        ...
        axes[0].set_ylabel('Power (dB)', fontsize=16, fontweight='bold')
        # Place legend in the corner of the Remembered panel that has the
        # least average data (so it doesn't obscure the curves).
