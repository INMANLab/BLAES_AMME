...
def plot_filtered_significance_bargraph(
    ...
    for band in power_order:
        ...
        add_method_caption(fig, full_caption)
        fig.tight_layout(rect=[0, 0.06 if use_rotated_ticks else 0, 1, 1])
        fig.savefig(os.path.join(out_dir, band_filename(filename, band)), dpi=300, bbox_inches='tight')
        finalize_figure(fig)
...
def build_trial_level_mlmr_export(df, diff_cols, measure_name='Coherence (Fisher Z)', excluded_substrings=None):
    if not diff_cols:
        return pd.DataFrame()
...
def build_retrieval_mlmr_export(data, measure_name='Coherence (Fisher Z)'):
...
def export_retrieval_mlmr_csv(data, csv_dir, file_stem, measure_name='Coherence (Fisher Z)'):
...
def plot_bc_bar_graph(data, out_dir, label):
    ...
    df_diff_bla_aug = _augment_with_bla_allhpc_diff(df_diff, data, freqs, POWER_RANGES)
    plot_filtered_significance_bargraph(
        df_diff_bla_aug,
        out_dir,
        f'{label} Retrieval Baseline-Corrected Coherency Diff (Stim - No Stim) - BLA Pairs',
        'Baseline-Corrected Coherence Diff',
        'Bargraph_baseline_corrected_coherencyDiff_byROI_retrieval_filtered_sig_BLApairs.png',
        rotate_xticks=True,
        allowed_pairs=COH_BLA_PANEL_PAIRS,
        caption_text='Method: For each patient and region pair, baseline-corrected spectra are averaged across all stim trials and all no-stim trials separately, band means are computed, then Stim-NoStim is taken. Bars = mean across patients, error bars = SEM, dots = patient values.',
    )
    plot_filtered_significance_bargraph(
        df_diff[df_diff['Region'].isin(unique_rois)],
        out_dir,
        f'{label} Retrieval Baseline-Corrected Coherency Diff (Stim - No Stim) - ALLHPC/EC/PRC Pairs',
        'Baseline-Corrected Coherence Diff',
        'Bargraph_baseline_corrected_coherencyDiff_byROI_retrieval_filtered_sig_nonBLApairs.png',
        rotate_xticks=True,
...
def plot_bc_bar_by_memory(data, out_dir, label):
    ...
    for mem, mem_label in [('remembered', 'Remembered'), ('forgotten', 'Forgotten')]:
        ...
        plot_filtered_significance_bargraph(
            df_mem_bla_aug,
            out_dir,
            f'{label} Retrieval Baseline-Corrected Coherency Diff (Stim - No Stim), {mem_label} Trials - BLA Pairs',
            'Baseline-Corrected Coherence Diff',
            f'Bargraph_baseline_corrected_coherencyDiff_byROI_retrieval_{mem}_filtered_sig_BLApairs.png',
            rotate_xticks=True,
            allowed_pairs=COH_BLA_PANEL_PAIRS,
            caption_text=f'Method: For each patient and region pair, baseline-corrected coherency spectra are averaged across stim and no-stim {mem_label.lower()} retrieval trials separately, band means are computed, then Stim-NoStim is taken. Bars = mean across patients, error bars = SEM, dots = patient values.',
        )
        plot_filtered_significance_bargraph(
            df_mem[df_mem['Region'].isin(unique_rois)],
            out_dir,
            f'{label} Retrieval Baseline-Corrected Coherency Diff (Stim - No Stim), {mem_label} Trials - ALLHPC/EC/PRC Pairs',
            'Baseline-Corrected Coherence Diff',
            f'Bargraph_baseline_corrected_coherencyDiff_byROI_retrieval_{mem}_filtered_sig_nonBLApairs.png',
            rotate_xticks=True,
            allowed_pairs=COH_NONBLA_PANEL_PAIRS,
            caption_text=f'Method: For each patient and region pair, baseline-corrected coherency spectra are averaged across stim and no-stim {mem_label.lower()} retrieval trials separately, band means are computed, then Stim-NoStim is taken. Bars = mean across patients, error bars = SEM, dots = patient values.',
        )
...
def plot_bc_remembered_forgotten(data, out_dir, label):
    ...
    for roi in visible_rois(all_rois, data.get('bc_plot_exclude_substrings', set())):
        ...
        rem_means = {'nostim': None, 'stim': None}
        ...
        loc = 'lower right'
        if rem_means['stim'] is not None and rem_means['nostim'] is not None:
            ...
            def _density(mask, y_low, y_high):
                if not np.any(mask):
                    return 0
                count = 0
                for series in (rem_means['stim'], rem_means['nostim']):
                    seg = series[mask]
        ...
        axes[0].legend(loc=loc, fontsize=12, framealpha=0.85)
...
if __name__ == '__main__':
    ...
    export_retrieval_mlmr_csv(blaes, CSV_OUTPUT_DIR, 'combined_retrieval_coherence_blaes_mlmr_input', measure_name='Coherence (Fisher Z)')
    export_retrieval_mlmr_csv(amme, CSV_OUTPUT_DIR, 'combined_retrieval_coherence_amme_mlmr_input', measure_name='Coherence (Fisher Z)')
    export_retrieval_mlmr_csv(all_data, CSV_OUTPUT_DIR, 'combined_retrieval_coherence_all_mlmr_input', measure_name='Coherence (Fisher Z)')
