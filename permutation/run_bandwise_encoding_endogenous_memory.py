...
(Remembered vs Forgotten) on no-stim ENCODING trials, per Region.

Vanilla only (no --balanced, no --no-bla, no --onesec).
Outputs: outputs/PermutationOutputsAlireza/encoding_bandwise/endogenous_memory/<modality>/.
"""
...
OUT_BASE = (ROOT / "outputs" / "PermutationOutputsAlireza"
            / "encoding_bandwise" / "endogenous_memory")
...
def run_modality(modality):
    ...
    fig.suptitle(
        f"Encoding | Endogenous memory (Remembered vs Forgotten) Band-wise - "
        f"{modality.upper()}\n"
        f"No-stim encoding trials, paired band permutation "
