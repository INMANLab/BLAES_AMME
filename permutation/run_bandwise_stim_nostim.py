#!/usr/bin/env python
"""Band-averaged paired permutation test of Stim vs No-stim retrieval
trials, split by Memory (Remembered / Forgotten), per Region.

Bands: delta(1.95-3), theta(4-8), alpha(9-13), beta(14-29),
slow_gamma(30-55), HFA(70-100). Confirmatory = {theta, slow_gamma};
Exploratory = {delta, alpha, beta, HFA}. Each family gets its own
null distribution of max |t| under sign-flipping permutation, mirroring
the per-frequency cluster-permutation FWE methodology.

Vanilla only (no --balanced, no --onesec).

Outputs: outputs/PermutationOutputsAlireza/retreival_bandwise/stim_nostim/<modality>/.
"""
...
OUT_BASE = (ROOT / "outputs" / "PermutationOutputsAlireza"
            / "retreival_bandwise" / "stim_nostim")
...
def run_modality(modality):
    ...
    fig.suptitle(
        f"Stim vs No-stim by Memory (Band-wise) - {modality.upper()}\n"
        f"Retrieval trials, paired band permutation "
