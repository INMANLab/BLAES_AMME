...
OUTPUT_ROOT = os.path.join(os.path.dirname(SCRIPT_DIR), 'OUTPUTS')
...
FILTERED_SELECTED_ROIS = {'ALLHPC', 'BLA', 'EC', 'PRC'}

COH_BLA_PANEL_PAIRS = [
    'BLA_ALLHPC', 'ALLHPC_BLA',
    'BLA_EC', 'EC_BLA',
    'BLA_PRC', 'PRC_BLA',
]
COH_NONBLA_PANEL_PAIRS = [
    'ALLHPC_EC', 'EC_ALLHPC',
    'ALLHPC_PRC', 'PRC_ALLHPC',
    'EC_PRC', 'PRC_EC',
]


def _augment_with_bla_allhpc_diff(df_diff, data, freqs, ranges, mem_key=None):
    try:
        comp = build_bla_composite_data(data)
    except Exception:
        return df_diff
    if mem_key == 'remembered':
        sd = comp.get('bc_stim_rem', {}); nd = comp.get('bc_nostim_rem', {})
    elif mem_key == 'forgotten':
        sd = comp.get('bc_stim_forg', {}); nd = comp.get('bc_nostim_forg', {})
    else:
        sd = comp.get('bc_stim', {}); nd = comp.get('bc_nostim', {})
    if not sd or not nd:
        return df_diff
    comp_diff = compute_diff_df(sd, nd, freqs, ranges)
    if comp_diff.empty:
        return df_diff
    bla_rows = comp_diff[comp_diff['Region'].isin({'BLA_ALLHPC', 'ALLHPC_BLA'})]
    if bla_rows.empty:
        return df_diff
    return pd.concat([df_diff, bla_rows], ignore_index=True)
...
def _filter_pair_regions(regions, selected):
    keep = []
    for r in regions:
        parts = str(r).split('_')
...
def plot_filtered_significance_bargraph(
    df_collapsed, out_dir, title, ylabel, filename,
    selected_regions=None, rotate_xticks=False, caption_text=None, is_pair=True,
    allowed_pairs=None,
):
    if df_collapsed.empty:
        return
    ...
    if allowed_pairs is not None:
        plot_df = plot_df[plot_df['Region'].isin(set(allowed_pairs))]
    elif is_pair:
        selected = set(selected_regions) if selected_regions is not None else FILTERED_SELECTED_ROIS
        keep = _filter_pair_regions(plot_df['Region'].unique(), selected)
        plot_df = plot_df[plot_df['Region'].isin(keep)]
    else:
        selected = set(selected_regions) if selected_regions is not None else FILTERED_SELECTED_ROIS
        plot_df = plot_df[plot_df['Region'].isin(selected)]
    if plot_df.empty:
        return
    power_order = [band for band in POWER_RANGES if band in plot_df['power_range'].unique()]
    if not power_order:
    ...
    if not roi_order:
        return
    use_rotated_ticks = rotate_xticks or len(roi_order) > 8 or any(len(str(roi)) > 8 for roi in roi_order)
    fig_width = max(11.5, min(20.0, 7.0 + 1.0 * len(roi_order)))
    ...
    for band in power_order:
        ...
        for roi in roi_order:
            vals = band_df.loc[band_df['Region'] == roi, 'mean_power_diff'].dropna().to_numpy()
        ...
        if valid.sum() > 0:
            adj[valid] = _bh_fdr(pvals[valid])

        fig, ax = plt.subplots(figsize=(max(fig_width, 13.0), 9.5))
        summary = band_df.groupby('Region')['mean_power_diff'].agg(['mean', 'sem']).reindex(roi_order)
        x = np.arange(len(roi_order))
        ...
        if 'Remembered Trials' in title:
            ...
            _trial = 'All Trials'
        ax.set_title(f'{_trial} - {band}', fontsize=30, fontweight='bold')
        ax.set_xlabel('')
        ax.set_xticks(x)
        ...
        ax.set_xticklabels(
            ...
            rotation=35 if use_rotated_ticks else 0,
            ha='right' if use_rotated_ticks else 'center',
            fontsize=18 if use_rotated_ticks else 21,
            fontweight='bold',
        )
        ax.tick_params(axis='y', labelsize=22, width=2, length=6)
        ax.tick_params(axis='x', width=2, length=6)
        ax.set_ylabel(ylabel, fontsize=27, fontweight='bold')
        full_caption = (caption_text or '') + (
            ' Significance: paired t-test (stim vs no-stim) per region pair, Benjamini-Hochberg FDR-corrected within band. '
            'Pairs with q<.05 are marked with a red bar border and a * next to the pair label.'
        )
        save_figure_output(
            fig,
            os.path.join(out_dir, band_filename(filename, band)),
            footer_text=full_caption,
            rect=[0, 0.06 if use_rotated_ticks else 0, 1, 1],
        )
...
def plot_grouped_condition_bars(df, out_dir, label, phase_label, filename_template, title_suffix, show_patients=True):
    ...
    for band in band_order:
        ...
        ax.set_ylabel('Mean Baseline-Corrected Coherence (Fisher Z)', fontsize=17, fontweight='bold')
...
def build_encoding_trial_level_export(df, diff_cols, measure_name='Coherence (Fisher Z)', excluded_substrings=None):
    if not diff_cols:
        return pd.DataFrame()
...
def build_encoding_mlmr_export(data, measure_name='Coherence (Fisher Z)'):
...
def export_encoding_mlmr_csv(data, csv_dir, file_stem, measure_name='Coherence (Fisher Z)'):
        ...
        if diff_cols:
            ...
            if not df_export.empty:
                ...
                export_df = build_encoding_trial_level_export(
                    df_export,
                    diff_cols,
                    measure_name='Coherence (Fisher Z)',
...
def load_amme_encoding():
    ...
    for file in power_files:
        ...
        # AMME-timing fix: drop Before-stim and During-stim encoding trials.
        if 'test_trial_type' in df.columns:
            df = df[~df['test_trial_type'].astype(str).isin(
...
def plot_stim_vs_nostim(data, out_dir, label, filename='GroupCoherency_byROI_encoding_stim_vs_nostim.png', footer_text=None):
    ...
    axes[0].set_ylabel('Coherence (Fisher Z)', fontsize=16, fontweight='bold')
...
def plot_bc_bar_graph(
    ...
    for band in [b for b in POWER_RANGES if b in df_diff['power_range'].unique()]:
        ...
        ax.set_xlabel('')
        ax.set_ylabel('Baseline-Corrected Coherence Diff', fontsize=16, fontweight='bold')
        ax.tick_params(axis='x', labelsize=14, width=2, length=6)
        ax.tick_params(axis='y', labelsize=14, width=2, length=6)
    ...
    plot_responder_status_bargraph(
        ...
        'Baseline-Corrected Coherence Diff',
        responder_filename,
        rotate_xticks=True,
    ...
    filtered_sig_stem, filtered_sig_ext = os.path.splitext(
        responder_filename.replace('_responder_status', '_filtered_sig')
        if '_responder_status' in responder_filename
        else responder_filename
    )
    if '_filtered_sig' not in filtered_sig_stem:
        filtered_sig_stem = filtered_sig_stem + '_filtered_sig'
    df_diff_bla_aug = _augment_with_bla_allhpc_diff(
        df_diff[df_diff['Region'].isin(unique_rois)],
        data, freqs, POWER_RANGES,
    )
    plot_filtered_significance_bargraph(
        df_diff_bla_aug,
        out_dir,
        f'{label} Encoding Baseline-Corrected Coherency Diff (Stim - No Stim) - BLA Pairs',
        'Baseline-Corrected Coherence Diff',
        f'{filtered_sig_stem}_BLApairs{filtered_sig_ext}',
        rotate_xticks=True,
        allowed_pairs=COH_BLA_PANEL_PAIRS,
        caption_text=(
            'Method: For each patient and region pair, baseline-corrected spectra are averaged across all '
            'stim trials and all no-stim trials separately, band means are computed, then Stim-NoStim is taken. '
            'Bars = mean across patients, error bars = SEM, dots = patient values.'
        ),
    )
    plot_filtered_significance_bargraph(
        df_diff[df_diff['Region'].isin(unique_rois)],
        out_dir,
        f'{label} Encoding Baseline-Corrected Coherency Diff (Stim - No Stim) - ALLHPC/EC/PRC Pairs',
        'Baseline-Corrected Coherence Diff',
        f'{filtered_sig_stem}_nonBLApairs{filtered_sig_ext}',
        rotate_xticks=True,
        allowed_pairs=COH_NONBLA_PANEL_PAIRS,
        caption_text=(
            'Method: For each patient and region pair, baseline-corrected spectra are averaged across all '
            'stim trials and all no-stim trials separately, band means are computed, then Stim-NoStim is taken. '
            'Bars = mean across patients, error bars = SEM, dots = patient values.'
...
def plot_bc_per_roi(
    ...
    for roi in all_rois:
        ...
        ax.set_xlabel('Frequency (Hz)', fontsize=16, fontweight='bold')
        ax.set_ylabel('Coherence (Fisher Z)', fontsize=16, fontweight='bold')
        ax.tick_params(axis='both', labelsize=14)
        ax.set_xlim(1, 100)
...
def plot_quadrant_stim_memory(data, out_dir, label, filename='Quadrant_StimMemory_Coherency_encoding.png', footer_text=None):
    ...
    fig.text(0.04, 0.5, 'Coherence (Fisher Z)', va='center', rotation='vertical', fontsize=18, fontweight='bold')
...
def plot_per_roi_quadrant(
    ...
    for roi in all_rois:
        ...
        fig.text(0.04, 0.5, 'Coherence (Fisher Z)', va='center', rotation='vertical', fontsize=14, fontweight='bold')
...
def plot_bc_bar_by_memory(
    ...
    for mem, mem_label in [('remembered', 'Remembered'), ('forgotten', 'Forgotten')]:
        ...
        df_mem_bla_aug = _augment_with_bla_allhpc_diff(
            df_mem[df_mem['Region'].isin(unique_rois)],
            data, freqs, POWER_RANGES, mem_key=mem,
        )
        plot_filtered_significance_bargraph(
            df_mem_bla_aug,
            out_dir,
            f'{label} Encoding Baseline-Corrected Coherency Diff (Stim - No Stim), {mem_label} Trials - BLA Pairs',
            'Baseline-Corrected Coherence Diff',
            f'Bargraph_baseline_corrected_coherencyDiff_byROI_encoding_{mem}_filtered_sig_BLApairs.png',
            rotate_xticks=True,
            allowed_pairs=COH_BLA_PANEL_PAIRS,
            caption_text=f'Method: For each patient and region pair, baseline-corrected coherency spectra are averaged across stim and no-stim {mem_label.lower()} trials separately, band means are computed, then Stim-NoStim is taken. Bars = mean across patients, error bars = SEM, dots = patient values.',
        )
        plot_filtered_significance_bargraph(
            df_mem[df_mem['Region'].isin(unique_rois)],
            out_dir,
            f'{label} Encoding Baseline-Corrected Coherency Diff (Stim - No Stim), {mem_label} Trials - ALLHPC/EC/PRC Pairs',
            'Baseline-Corrected Coherence Diff',
            f'Bargraph_baseline_corrected_coherencyDiff_byROI_encoding_{mem}_filtered_sig_nonBLApairs.png',
            rotate_xticks=True,
            allowed_pairs=COH_NONBLA_PANEL_PAIRS,
            caption_text=f'Method: For each patient and region pair, baseline-corrected coherency spectra are averaged across stim and no-stim {mem_label.lower()} trials separately, band means are computed, then Stim-NoStim is taken. Bars = mean across patients, error bars = SEM, dots = patient values.',
        )
        ...
        for band in [b for b in POWER_RANGES if b in df_mem['power_range'].unique()]:
            ...
            sns.stripplot(data=sub, x='Region', y='mean_power_diff', ax=ax,
                          color='black', alpha=0.5, jitter=0.2, order=unique_rois)
            ax.set_xlabel('')
            ax.set_ylabel('Coherence Diff', fontsize=16, fontweight='bold')
...
def plot_coherence_by_roi_core(data, out_dir, label):
    ...
    ax.set_xlabel('Frequency (Hz)', fontsize=18, fontweight='bold')
    ax.set_ylabel('Coherence (Fisher Z)', fontsize=18, fontweight='bold')
    ax.set_title(f'{label} Encoding Group Coherency by ROI (Hipp Subregions)', fontsize=20, fontweight='bold')
    ax.tick_params(axis='both', labelsize=14)
    ax.legend(bbox_to_anchor=(1.02, 0.5), loc='center left', prop={'weight': 'bold', 'size': 12})
    plt.tight_layout(rect=[0, 0, 0.85, 1])
    plt.savefig(os.path.join(out_dir, 'GroupCoherency_byROI_encoding_core_regions.png'), dpi=300, bbox_inches='tight')
    finalize_figure()
...
def plot_freq_x_memory_coherence_bargraph(data, out_dir, label):
    ...
    # Remove confirmed outlier: amyg033 BLA_CA Theta forgotten (~-1.27 diff)
    df_all = df_all[~(
        (df_all['Patient'] == 'amyg033')
        & (df_all['Region'] == 'BLA_CA')
    ...
    fig, axes = plt.subplots(
        1,
        len(band_order),
        figsize=(max(13.5, len(roi_order) * 1.6 * len(band_order)), 8.2),
        sharey=True,
    )
    if len(band_order) == 1:
        axes = [axes]
    ...
    axes[0].set_ylabel('Coherence Diff (Stim − No Stim)', fontsize=16, fontweight='bold')
    ...
    fig.suptitle(f'{label} Encoding Frequency × Memory (Coherence) Boxplots',
                 fontsize=20, fontweight='bold', y=0.98)
    add_method_caption(
        fig,
        'Method: For each patient and ROI pair, band diff = Stim−NoStim. Panels separate theta, slow gamma, and HFA. '
...
def plot_bc_quadrant_stim_memory(data, out_dir, label):
    ...
    fig.text(0.5, 0.04, 'Frequency (Hz)', ha='center', fontsize=18, fontweight='bold')
    fig.text(0.04, 0.5, 'Baseline-Corrected Coherence (Fisher Z)', va='center', rotation='vertical', fontsize=18, fontweight='bold')
...
if __name__ == '__main__':
    ...
    export_encoding_mlmr_csv(blaes, CSV_OUTPUT_DIR, 'combined_encoding_coherence_blaes_mlmr_input', measure_name='Coherence (Fisher Z)')
    export_encoding_mlmr_csv(amme, CSV_OUTPUT_DIR, 'combined_encoding_coherence_amme_mlmr_input', measure_name='Coherence (Fisher Z)')
    export_encoding_mlmr_csv(all_data, CSV_OUTPUT_DIR, 'combined_encoding_coherence_all_mlmr_input', measure_name='Coherence (Fisher Z)')
