...
def plot_filtered_significance_bargraph(
    ...
    selected_regions=None, rotate_xticks=False, caption_text=None, is_pair=False,
):
    if df_collapsed.empty:
        return
    ...
    for band in power_order:
        ...
        full_caption = (caption_text or '') + (
            ' Significance: paired t-test (stim vs no-stim) per region, Benjamini-Hochberg FDR-corrected within band. '
            'Regions with q<.05 are marked with a red bar border and a * next to the region label.'
        )
...
def plot_grouped_condition_bars(df, out_dir, label, phase_label, filename_template, title_suffix, show_patients=True):
    ...
    for band in band_order:
        ...
        if show_patients:
            ...
            fig.subplots_adjust(left=0.10, right=0.95, bottom=0.15, top=0.86)
...
def build_encoding_trial_level_export(df, diff_cols, measure_name='Power'):
    if not diff_cols:
        return pd.DataFrame()
...
def build_encoding_mlmr_export(data, measure_name='Power'):
...
def export_encoding_mlmr_csv(data, csv_dir, file_stem, measure_name='Power'):
    ...
    for file in power_files:
        ...
        if diff_cols:
            ...
            if not df_export.empty:
                ...
                export_df = build_encoding_trial_level_export(df_export, diff_cols, measure_name='Power')
...
def load_amme_encoding():
    ...
    for file in power_files:
        ...
        # AMME-timing fix: drop Before-stim and During-stim encoding trials so
        # only nostim and After-stim (= clean post-stim) trials enter analysis.
        # Applies to amyg045-amyg072 where test_trial_type carries the four
        # timing labels; older AMME patients only have "stim"/"nostim" so are
...
def plot_bc_bar_graph(data, out_dir, label):
    ...
    for band in power_order:
        ...
        ax.set_title(band, fontsize=18, fontweight='bold')
        fig.suptitle(f'{label} Encoding Baseline-Corrected Power Diff (Stim - No Stim)',
                     fontsize=20, fontweight='bold', y=0.98)
        fig.tight_layout(rect=[0, 0, 1, 0.9])
        plt.savefig(
            os.path.join(out_dir, band_filename('Bargraph_baseline_corrected_powerDiff_byROI_encoding.png', band)),
            bbox_inches='tight',
            dpi=300,
        )
        finalize_figure(fig)
    ...
    plot_grouped_condition_bars(
        ...
        label,
        'Encoding',
        'StimVsNoStim_alltrials_encoding_clean.png',
        'All Trials',
        show_patients=False,
    )
    plot_responder_status_bargraph(
        ...
        out_dir,
        f'{label} Encoding Baseline-Corrected Power Diff (Stim - No Stim)',
        'Baseline-Corrected Power Diff',
        'Bargraph_baseline_corrected_powerDiff_byROI_encoding_responder_status.png',
    ...
    plot_filtered_significance_bargraph(
        ...
        f'{label} Encoding Baseline-Corrected Power Diff (Stim - No Stim)',
        'Baseline-Corrected Power Diff',
        'Bargraph_baseline_corrected_powerDiff_byROI_encoding_filtered_sig.png',
        caption_text='Method: For each patient and ROI, baseline-corrected spectra are averaged across all stim trials and all no-stim trials separately, band means are computed, then Stim-NoStim is taken. Bars = mean across patients, error bars = SEM, dots = patient values.',
    )
...
def plot_bc_bar_by_memory(data, out_dir, label):
    ...
    for mem, mem_label in [('remembered', 'Remembered'), ('forgotten', 'Forgotten')]:
        ...
        plot_responder_status_bargraph(
            ...
            out_dir,
            f'{label} Encoding Baseline-Corrected Power Diff (Stim - No Stim), {mem_label} Trials',
            'Baseline-Corrected Power Diff',
            f'Bargraph_baseline_corrected_powerDiff_byROI_encoding_{mem}_responder_status.png',
            caption_text=f'Method: For each patient and ROI, baseline-corrected spectra are averaged across stim and no-stim {mem_label.lower()} trials separately, band means are computed, then Stim-NoStim is taken. Bars = mean across patients, error bars = SEM, dots = patient values colored by responder-status CSV.',
        )
        plot_filtered_significance_bargraph(
            df_mem[df_mem['Region'].isin(unique_rois)],
            out_dir,
            f'{label} Encoding Baseline-Corrected Power Diff (Stim - No Stim), {mem_label} Trials',
            'Baseline-Corrected Power Diff',
            f'Bargraph_baseline_corrected_powerDiff_byROI_encoding_{mem}_filtered_sig.png',
        ...
        for band in power_order:
            ...
            ax.axhline(0, color='grey')
            ax.set_title(band, fontsize=18, fontweight='bold')
            fig.suptitle(f'{label} Encoding Power Diff, {mem_label} Trials',
                         fontsize=20, fontweight='bold', y=0.98)
            fig.tight_layout(rect=[0, 0, 1, 0.9])
            plt.savefig(
                os.path.join(out_dir, band_filename(f'Bargraph_Power_diff_{mem}_encoding.png', band)),
                bbox_inches='tight',
                dpi=300,
            )
            finalize_figure(fig)
...
def plot_bc_quadrant_stim_memory(data, out_dir, label):
    ...
    plt.savefig(os.path.join(out_dir, 'Quadrant_BaselineCorrected_StimMemory_Power_encoding.png'),
...
def plot_connected_dots(data, out_dir, label, show_patients=True):
    ...
    for mem, mem_label in [('remembered', 'Remembered'), ('forgotten', 'Forgotten')]:
        ...
        for band in band_order:
            ...
            plt.savefig(os.path.join(out_dir, band_filename(f'StimVsNoStim_{mem}_encoding{clean_suffix}.png', band)),
...
def plot_power_by_roi_core(data, out_dir, label):
    ...
    ax.set_xlabel('Frequency (Hz)', fontsize=18, fontweight='bold')
    ax.set_ylabel('Power (dB)', fontsize=18, fontweight='bold')
    ax.set_title(f'{label} Encoding Group Power by ROI (Hipp Subregions)', fontsize=20, fontweight='bold')
    ax.tick_params(axis='both', labelsize=14)
    ax.legend(bbox_to_anchor=(1.02, 0.5), loc='center left', prop={'weight': 'bold', 'size': 12})
    plt.tight_layout(rect=[0, 0, 0.85, 1])
...
if __name__ == '__main__':
    ...
    all_data = build_all_encoding_data(blaes, amme)

    print("\nExporting encoding MLMR CSVs...")
    ensure_dir(CSV_OUTPUT_DIR)
    export_encoding_mlmr_csv(blaes, CSV_OUTPUT_DIR, 'combined_encoding_power_blaes_mlmr_input', measure_name='Power')
    export_encoding_mlmr_csv(amme, CSV_OUTPUT_DIR, 'combined_encoding_power_amme_mlmr_input', measure_name='Power')
    export_encoding_mlmr_csv(all_data, CSV_OUTPUT_DIR, 'combined_encoding_power_all_mlmr_input', measure_name='Power')
