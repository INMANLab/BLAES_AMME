...
def plot_pac_by_roi(data, out_dir, label='BLAES'):
    ...
    fig.savefig(out_dir / 'GroupPAC_byROI_encoding.png', dpi=300, bbox_inches='tight')
    finalize_figure(fig)
...
def plot_pac_by_patient(data, out_dir, label='BLAES'):
    ...
    ax.legend(bbox_to_anchor=(1.02, 0.5), loc='center left', fontsize=9, frameon=False)
    fig.tight_layout(rect=[0, 0, 0.82, 1])
    fig.savefig(out_dir / 'GroupPAC_byPatient_encoding.png', dpi=300, bbox_inches='tight')
    finalize_figure(fig)
...
def plot_stim_vs_nostim(data, out_dir, label='BLAES'):
    ...
    fig.savefig(out_dir / 'GroupPAC_byROI_encoding_stim_vs_nostim.png', dpi=300, bbox_inches='tight')
    finalize_figure(fig)
...
def plot_per_roi_patient_stim_nostim(data, out_dir, label='BLAES'):
    ...
    for roi in all_rois:
        ...
        if plotted_any:
            ...
            fig.savefig(out_dir / f'GroupPAC_{roi}_byPatient_encoding_stim_vs_nostim.png', dpi=300, bbox_inches='tight')
        finalize_figure(fig)
...
def plot_bc_bar_graph(data, out_dir, label='BLAES'):
    ...
    for band in [name for name in PAC_BANDS if name in diff_df['band'].unique()]:
        ...
        fig.savefig(out_dir / f'Bargraph_baseline_corrected_PACDiff_byROI_encoding_{band.lower().replace(" ", "_")}.png', dpi=300, bbox_inches='tight')
        finalize_figure(fig)
...
def plot_mi_difference_per_roi(data, out_dir, label='BLAES'):
    ...
    for roi in sorted(set(stim_dict.keys()) | set(nostim_dict.keys())):
        ...
        style_axis(ax, hide_top_right=True)
        fig.tight_layout()
        fig.savefig(out_dir / f'{roi}_MI_difference_encoding.png', dpi=300, bbox_inches='tight')
        finalize_figure(fig)
...
def plot_bc_per_roi(data, out_dir, label='BLAES'):
    ...
    for roi in sorted(set(stim_dict.keys()) | set(nostim_dict.keys())):
        ...
        fig.tight_layout()
        fig.savefig(out_dir / f'{roi}_BaselineCorrectedPAC_encoding_stim_vs_nostim.png', dpi=300, bbox_inches='tight')
        finalize_figure(fig)
...
def plot_quadrant_stim_memory(data, out_dir, label='BLAES'):
    ...
    fig.savefig(out_dir / 'Quadrant_StimMemory_PAC_encoding.png', dpi=300, bbox_inches='tight')
    finalize_figure(fig)
...
def plot_per_roi_quadrant(data, out_dir, label='BLAES'):
    ...
    for roi in all_rois:
        ...
        if has_data:
            ...
            fig.savefig(out_dir / f'IndivQuadrant_{roi}_StimMemory_PAC_encoding.png', dpi=300, bbox_inches='tight')
        finalize_figure(fig)
...
def plot_bc_bar_by_memory(data, out_dir, label='BLAES'):
    ...
    for memory_cond, memory_label in [('remembered', 'Remembered'), ('forgotten', 'Forgotten')]:
        ...
        for band in [name for name in PAC_BANDS if name in subset['band'].unique()]:
            ...
            fig.tight_layout(rect=[0, 0, 1, 0.92])
            fig.savefig(out_dir / f'Bargraph_PAC_diff_{memory_cond}_encoding_{band.lower().replace(" ", "_")}.png', dpi=300, bbox_inches='tight')
            finalize_figure(fig)
...
def plot_bc_remembered_forgotten(data, out_dir, label='BLAES'):
    ...
    for roi in sorted(all_rois):
        ...
        if has_data:
            ...
            fig.savefig(out_dir / f'{roi}_Baseline_Adjusted_PAC_RememberedForgotten_encoding.png', dpi=300, bbox_inches='tight')
        finalize_figure(fig)
