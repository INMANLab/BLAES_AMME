...
def plot_bc_bar_graph(
    ...
    plot_grouped_condition_bars(
        ...
        label,
        'Retrieval',
        'StimVsNoStim_alltrials_retrieval_{band}.png',
        'All Trials',
        footer_text=footer_text,
    )
    plot_grouped_condition_bars(
    ...
    plot_responder_status_bargraph(
        ...
        out_dir,
        f'{label} Retrieval Baseline-corrected PAC Diff (Stim - No Stim)',
        'Baseline-corrected PAC Diff',
        'Bargraph_baseline_corrected_PACDiff_byROI_retrieval_responder_status.png',
        rotate_xticks=True,
...
def plot_connected_dots(data, out_dir, label, footer_text=None, show_patients=True):
    ...
    for mem, mem_label in [('remembered', 'Remembered'), ('forgotten', 'Forgotten')]:
        ...
        for band in band_order:
            ...
            base_fn = band_filename(f'StimVsNoStim_{mem}_retrieval.png', band)
...
def plot_mi_difference_per_roi(
    ...
    for roi in sorted(all_rois):
        ...
        fig.suptitle(f'{label} Retrieval {roi} MI Difference (Stim - No Stim)', fontsize=18, fontweight='bold')
        save_figure_output(
            fig,
            out_dir / filename_template.format(roi=roi),
            footer_text=footer_text,
...
def plot_bc_bar_by_memory(
    ...
    for memory_cond, memory_label in [('remembered', 'Remembered'), ('forgotten', 'Forgotten')]:
        ...
        plot_grouped_condition_bars(
            ...
            label,
            'Retrieval',
            f'StimVsNoStim_{memory_cond}_retrieval_{{band}}.png',
