#!/usr/bin/env python
"""2x2 figure: theta power x prior BLA stimulation interaction for each
region in the BLAMTL family (ALLHPC, BLA, EC, PRC). Same v4 style as the
PRC single-panel: GLMM line + model-based 95% CI shading + 10-bin overlay.

Stats source = the per-region single-band GLMM from run_h1abc_full_retrieval.R
(which produced the FDR-corrected BLAMTL PDF). PRC is the only panel that
survives FDR within the BLAMTL theta family (q_FDR = 0.014) and gets a *
annotation near (0, 0.97).
"""
...
PANELS = [
    ("ALLHPC", 0.102, 0.176, False),
    ("BLA",    0.132, 0.176, False),
    ("EC",     0.308, 0.308, False),
    ("PRC",    0.004, 0.014, True),
]
...
def draw_panel(ax, region, p_raw, q_fdr, mark_star):
    ...
    pred = pd.read_csv(GLMM_DIR / f"power_{region}_theta_predictions.csv")
...
def main():
    ...
    fig.text(
        ...
        "error bars show binomial SE of the bin mean. * = survives FDR within "
        "the BLAMTL theta family.",
        ha="center", va="bottom", fontsize=9, style="italic", wrap=True,
    )
    fig.tight_layout(rect=[0, 0.04, 1, 1])

    out_path = FIG_DIR / "BLAMTL_theta_interactions_2x2.png"
