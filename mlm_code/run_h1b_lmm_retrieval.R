##############################################################################
# Hypothesis 1b - LMM analyses (neural feature as outcome)
#   Tests whether stim-induced changes in retrieval-period neural features
#   between BLA, HPC, EC, PRC differ between participants whose memory was
#   enhanced, unchanged, or impaired by stim.
#
# Per region (power) or pair (coherence/PAC), per band:
#   feature ~ StimCond * MemoryGroup + (1 | Patient)
#
# Two moderator flavors run in parallel:
#   continuous = z-scored avg_stim_dprime_diff (per subject)
#   categorical = Enhanced (Strong+Moderate) | Non-responder (ref) | Impaired (Anti)
#
# Bands (power/coherence): theta, slow_gamma. PAC type: SG PAC only.
# Pairs/regions restricted per H1b scope (BLA, HPC, EC, PRC + their pairs).
#
# Usage: Rscript run_h1b_lmm_retrieval.R --imbalanced
##############################################################################
...
out_dir <- file.path(script_dir, "outputs", paste0("h1b_lmm_", label, "_retrieval_mlm"))
...
# BJH032 == BJH033 (same subject, two IDs in raw data); trial data uses
# BJH032. Fold BJH033 -> BJH032 and dedupe so we end up with 54 unique IDs.
resp$Patient[resp$Patient == "BJH033"] <- "BJH032"
resp <- resp[!duplicated(resp$Patient), ]
resp$ResponderGroup <- factor(
  ifelse(resp$ResponderStatus %in% c("Strong responders", "Moderate responders"),
         "Enhanced",
         ifelse(resp$ResponderStatus == "Anti-responders", "Impaired",
                "Non_responder")),
  levels = c("Non_responder", "Enhanced", "Impaired")
)
resp$mem_mod_z <- as.numeric(scale(resp$avg_stim_dprime_diff))

cat(sprintf("Responder coverage: %s\n",
            paste(sprintf("%s=%d", levels(resp$ResponderGroup),
                          tabulate(resp$ResponderGroup)),
                  collapse=", ")))
...
# Refit nested models with REML=FALSE for LR tests; keep REML for the headline coefs.
safe_lmer_ML <- function(formula, data) {
  tryCatch(lmer(formula, data = data, REML = FALSE, control = ctrl),
...
filter_balanced <- function(df, lab) {
  ...
  cnt <- aggregate(yes_or_no ~ Patient,
                   data=df[df$yes_or_no %in% c("yes","no"), ],
                   FUN = function(x) c(yes=sum(x=="yes"), no=sum(x=="no")))
...
prepare_trial_df <- function(csv_path, lab) {
  ...
  # Merge responder info
  d <- merge(d, resp[, c("Patient", "ResponderGroup", "mem_mod_z")],
...
run_unit <- function(sub, feature_col, modality, unit, band_label) {
  # Drop rows missing the moderator(s) or the feature.
  ...
  # ── Continuous moderator (mem_mod_z) ──
  fmls_cont <- list(
  ...
  if (!is.null(fits$m_full)) {
    ...
    if (!is.null(full_reml)) {
      save_coefs(full_reml, sprintf("%s_%s_%s_modContinuous_coefs.csv",
  ...
  # ── Categorical moderator (ResponderGroup) ──
  fmls_cat <- list(
    m0     = "feature ~ 1 + (1 | Patient)",
    m1     = "feature ~ StimCond + (1 | Patient)",
  ...
  if (!is.null(fits2$m_full)) {
    ...
    if (!is.null(full_reml2)) {
      save_coefs(full_reml2, sprintf("%s_%s_%s_modCategorical_coefs.csv",
                                     modality, unit, band_label))
    }
    save_model_build(fits2, fmls_cat, sprintf("%s_%s_%s_modCategorical_anova.csv",
                                              modality, unit, band_label))
  }
}
...
for (pair in COH_PAIRS) {
  ...
  for (b in names(BANDS)) {
    run_unit(sub, b, "coherence", pair, b)
  }
}
