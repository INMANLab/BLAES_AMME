##############################################################################
# Hypothesis 2a — Encoding LMM analyses (primary between-subject tests).
#   Mirrors the primary tests from run_h1b_subregions_quad_lmm_retrieval.R
#   but for ENCODING data.
#
# H2a: Individuals with weak baseline functional connectivity in theta and
# gamma between BLA and other MTL regions prior BLA-stimulation will show
# memory enhancement at recognition; the opposite for individuals with
# strong baseline coupling.
#
# For each region (power) or pair (coherence/PAC), per band, fit:
#   feature ~ Memory Modulation Z + (1 | Patient)
#   feature ~ QuadResponderGroup    + (1 | Patient)
#
# Inputs:
#   combined_encoding_<modality>_all_mlmr_input.csv (AMME-timing filtered)
#
# Outputs to outputs/h2a_subregions_quad_lmm_<filter>_encoding_mlm/
#
# BJH032 == BJH033 -> folded together (54 unique patients).
# Usage:
#   Rscript run_h2a_subregions_quad_lmm_encoding.R --imbalanced
##############################################################################
...
out_dir <- file.path(script_dir, "outputs",
                     paste0("h2a_subregions_quad_lmm_", label, "_encoding_mlm"))
...
PAC_PAIRS <- c("BLA_CA","BLA_DG","BLA_HPC","BLA_EC","BLA_PRC",
               "CA_EC","CA_PRC","DG_EC","DG_PRC","EC_HPC","HPC_PRC")

# Responder status / memory modulation Z
...
run_unit <- function(sub, feature_col, modality, unit, band_label) {
  ...
  # ── Primary H2a tests (between-subject only, no StimCond) ────────────────
  fmls_pri_mem <- list(
...
##############################################################################
cat("\n", paste(rep("=",70),collapse=""), "\nH2A LMM POWER (encoding) -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
pw <- prepare_trial_df(file.path(script_dir, "outputs", "csvs",
                                 "combined_encoding_power_all_mlmr_input.csv"), "Power")
...
cat("\n", paste(rep("=",70),collapse=""), "\nH2A LMM COHERENCE (encoding) -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
coh <- prepare_trial_df(file.path(script_dir, "outputs", "csvs",
                                  "combined_encoding_coherence_all_mlmr_input.csv"), "Coherence")
...
cat("\n", paste(rep("=",70),collapse=""), "\nH2A LMM PAC (encoding, SG) -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
pac <- prepare_trial_df(file.path(script_dir, "outputs", "csvs",
                                  "combined_encoding_pac_all_mlmr_input.csv"), "PAC")
