##############################################################################
# Hypothesis 1b - GLMM analyses (binomial accuracy, 3-way interaction)
#   Tests whether the trial-level brain-behavior coupling between baseline
#   neural features and memory differs as a function of stimulation AND of
#   the participant's overall memory modulation by stim.
#
...
#
# Usage: Rscript run_h1b_subregions_glmm_retrieval.R --imbalanced
##############################################################################
...
out_dir <- file.path(script_dir, "outputs", paste0("h1b_subregions_glmm_", label, "_retrieval_mlm"))
...
# Responder status load (BJH032 == BJH033, fold)
...
safe_glmer <- function(formula, data) {
  tryCatch(glmer(formula, data = data, family = binomial, control = ctrl),
...
run_unit <- function(sub, feature_col, modality, unit, band_label) {
  ...
  fmls_cat <- list(
    m0     = "Accuracy ~ 1 + (1 | Patient)",
    m1     = "Accuracy ~ StimCond + feature_c + ResponderGroup + (1 | Patient)",
    m2     = "Accuracy ~ StimCond * feature_c + ResponderGroup + StimCond:ResponderGroup + feature_c:ResponderGroup + (1 | Patient)",
    m_full = "Accuracy ~ StimCond * feature_c * ResponderGroup + (1 | Patient)"
  )
  fits2 <- list()
  for (nm in names(fmls_cat)) fits2[[nm]] <- safe_glmer(as.formula(fmls_cat[[nm]]), sub)
  if (!is.null(fits2$m_full)) {
    save_coefs(fits2$m_full, sprintf("%s_%s_%s_modCategorical_coefs.csv",
                                      modality, unit, band_label))
...
# COHERENCE
##############################################################################
cat("\n", paste(rep("=",70),collapse=""), "\nH1B GLMM COHERENCE -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
