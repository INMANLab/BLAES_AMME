#!/usr/bin/env python
"""
Build a Hypothesis 1b PDF report from coefs/anovas produced by either
run_h1b_subregions_lmm_retrieval.R or run_h1b_subregions_glmm_retrieval.R.

Usage:
  python build_h1b_subregions_report.py <lmm|glmm> <balanced|imbalanced>

Layout per band/region/pair: continuous-moderator full model + sequential
build, then categorical-moderator full model + sequential build.
"""
...
if len(sys.argv) < 3:
    print("Usage: python build_h1b_subregions_report.py <lmm|glmm> <balanced|imbalanced>")
    sys.exit(1)
...
DATA_DIR = os.path.join(SCRIPT_DIR, "outputs", f"h1b_subregions_{KIND}_{FILTER}_retrieval_mlm")
FILTER_LABEL = "Balanced Trials" if FILTER == "balanced" else "ImBalanced Trials"

REPORT_TITLE = f"Hypothesis 1b ({KIND_LABEL}) - Retrieval - {FILTER_LABEL}"
OUTPUT_PDF = os.path.join(
    SCRIPT_DIR, "outputs",
    f"Hypothesis_1b_HippSubregions_Report_{KIND.upper()}_Retrieval_{FILTER_LABEL.replace(' ', '_')}.pdf",
)
...
PAC_PAIRS = ["BLA_CA","BLA_DG","BLA_HPC","BLA_EC","BLA_PRC"]
...
def render_panel(pdf, title_prefix, modality, unit, band_label):
    """Render the two-moderator panels for one (modality, unit, band)."""
    has_any = False
    for mod_kind, mod_label in [("Continuous", "memory modulation (z-scored stim-no-stim d')"),
                                ("Categorical", "responder group (Enhanced/Non/Impaired)")]:
...
def build_report():
    ...
    pdf.multi_cell(0, 9,
                   "Hypothesis 1b\n"
                   f"Stim-induced neural changes by memory-modulation group ({KIND_LABEL})")
    pdf.ln(2)
    ...
    pdf.body_text(
        "Two moderators are evaluated in parallel for every panel: "
        "(1) continuous memory modulation = z-scored avg_stim_dprime_diff "
        "(stim - no-stim d'); "
        "(2) categorical responder status, collapsed to three levels with "
        "Non-responder as reference. Subject IDs BJH032 and BJH033 are "
        "folded to a single subject (54 unique participants)."
    )
    pdf.body_text(
        "Categorical group definitions and ranges of avg_stim_dprime_diff:"
    )
    pdf.apa_table(
        "Table. Responder group definitions (3-level)",
        ["Group", "Source labels", "n", "min", "median", "max"],
        [
            ["Impaired", "Anti-responders", "14", "-0.779", "-0.306", "-0.139"],
            ["Non_responder (ref)", "Non-responders", "13", "-0.130", "0.000", "+0.076"],
            ["Enhanced", "Moderate + Strong responders", "27", "+0.077", "+0.250", "+0.840"],
        ],
        col_widths=[36, 60, 14, 22, 22, 22],
        note="d' difference range. Non-responder is the reference category.",
    )
    ...
    if KIND == "lmm":
        pdf.body_text(
            ...
            "neural feature is regressed on stim condition x moderator with "
            "a patient random intercept. Model: feature ~ StimCond * "
            "MemoryGroup + (1|Patient). The StimCond x MemoryGroup "
            "interaction is the direct H1b test: it asks whether "
            "stim-induced change in the neural feature differs as a "
            "function of memory group."
        )
        pdf.body_text(
            "Fixed effects fit with REML; sequential model comparisons "
            "(m0 -> +StimCond -> +moderator -> +interaction) refit with "
            "ML for likelihood-ratio tests."
        )
    else:
        pdf.body_text(
            "Per region (power) or pair (coherence/PAC), per band, "
            "trial-level binomial GLMMs test the 3-way interaction: "
            "Accuracy ~ StimCond * feature_c * MemoryGroup + (1|Patient). "
            "Sequential model build adds main effects, all 2-way "
            "interactions, then the 3-way term. The 3-way interaction "
            "tests whether the brain-behavior coupling between baseline "
            "neural feature and memory differs by stimulation AND memory "
            "group."
        )
    pdf.body_text(
        "Bands tested: theta (4-8 Hz) and slow gamma (30-55 Hz). PAC: "
        "SG PAC (30-50 Hz) only. Per-panel patients >= 5 required."
    )
    pdf.body_text(
        "Power regions: BLA, HPC, EC, PRC. Coherence pairs: BLA-HPC, BLA-EC, "
        "BLA-PRC, EC-HPC, EC-PRC, HPC-PRC. PAC pairs: BLA-HPC, BLA-EC, "
        "BLA-PRC."
    )
    ...
    for reg in POWER_REGIONS:
        for bk, bl in BAND_LABELS.items():
            ...
            anchor_check = load_csv(f"power_{reg}_{bk}_modContinuous_coefs.csv")
    ...
    for pair in COH_PAIRS:
        for bk, bl in BAND_LABELS.items():
            ...
            anchor_check = load_csv(f"coherence_{pair}_{bk}_modContinuous_coefs.csv")
            if anchor_check is None:
                continue
            pdf.subsection_title(f"{sec}.{sub_idx}. {title_prefix}")
            sub_idx += 1
    ...
    for pair in PAC_PAIRS:
        ...
        anchor_check = load_csv(f"pac_{pair}_slow_gamma_modContinuous_coefs.csv")
        if anchor_check is None:
            continue
        pdf.subsection_title(f"{sec}.{sub_idx}. {title_prefix}")
        sub_idx += 1
