...
#
# Encoding bands: theta and slow gamma only (for power/coherence)
...
# Encoding bands: theta, slow gamma, and HFA (fast gamma).
BANDS <- list(
...
# ── NEW: Per-region multi-band power model (theta + slow gamma + HFA) ──
# Predictor entry order: StimCond -> theta_c -> slow_gamma_c -> fast_gamma_c
#                        -> band:StimCond interactions.
cat("\n--- NEW: Per-Region Multi-Band Power Models (>=5 patients) ---\n")
for (reg in all_regions) {
  sub <- pw[pw$Region == reg, ]
  sub$Patient <- factor(sub$Patient)
  ...
  sub$fast_gamma_c <- sub$fast_gamma - mean(sub$fast_gamma, na.rm=TRUE)
  cat(sprintf("\n  Region %s (multi-band): %d trials, %d patients\n",
              reg, nrow(sub), nlevels(sub$Patient)))
  fmls <- list(
    m0     = "Accuracy ~ 1 + (1 | Patient)",
    ...
    m4     = "Accuracy ~ StimCond + theta_c + slow_gamma_c + fast_gamma_c + (1 | Patient)",
    m_full = "Accuracy ~ StimCond + theta_c + slow_gamma_c + fast_gamma_c + theta_c:StimCond + slow_gamma_c:StimCond + fast_gamma_c:StimCond + (1 | Patient)"
  )
  fits <- list()
...
# ── NEW: Per-pair multi-band coherence model (theta + slow gamma + HFA, BLA-X pairs, >=5 patients) ──
cat("\n--- NEW: Per-Pair Multi-Band Coherence Models (BLA-X) ---\n")
for (pair in BLA_PAIRS_COH) {
  ...
  sub$slow_gamma_c <- sub$slow_gamma - mean(sub$slow_gamma, na.rm=TRUE)
  sub$fast_gamma_c <- sub$fast_gamma - mean(sub$fast_gamma, na.rm=TRUE)
  cat(sprintf("\n  Pair %s (multi-band): %d trials, %d patients\n",
