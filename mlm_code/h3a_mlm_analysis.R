...
save_diagnostics(m1a, "mlm_model1a_diagnostics.csv")

# Fit null model for LRT
m_null <- glmer(memory ~ 1 + (1 | patient_id),
                data = d, family = binomial(link = "logit"),
                control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
save_diagnostics(m_null, "mlm_null_diagnostics.csv")
save_lrt(m_null, m1a, "lrt_h3a_m1a_vs_null.csv")
...
save_diagnostics(m1b, "mlm_model1b_diagnostics.csv")

# Compare models
cat("\n--- Model comparison: 1a vs 1b ---\n")
print(anova(m1a, m1b))
save_lrt(m1a, m1b, "lrt_h3a_m1b_vs_m1a.csv")
...
save_diagnostics(m2a, "mlm_model2a_diagnostics.csv")

# Null model for model 2 (different subset - patients with retrieval data)
m2_null <- glmer(memory ~ 1 + (1 | patient_id),
                 data = d2, family = binomial(link = "logit"),
                 control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
save_lrt(m2_null, m2a, "lrt_h3a_m2a_vs_null.csv")
...
save_diagnostics(m2b, "mlm_model2b_diagnostics.csv")
