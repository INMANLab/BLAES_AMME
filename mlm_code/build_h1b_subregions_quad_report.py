...
Reads coefs/anovas from outputs/h1b_subregions_quad_<lmm|glmm>_<filter>_retrieval_mlm/
and writes outputs/Hypothesis_1b_HippSubregions_QuadResponderCategory_<KIND>_Retrieval_*.pdf.

Usage:
  python build_h1b_subregions_quad_report.py <lmm|glmm> <balanced|imbalanced>
"""
...
if len(sys.argv) < 3:
    print("Usage: python build_h1b_subregions_quad_report.py <lmm|glmm> <balanced|imbalanced>")
    sys.exit(1)
...
DATA_DIR = os.path.join(SCRIPT_DIR, "outputs",
                        f"h1b_subregions_quad_{KIND}_{FILTER}_retrieval_mlm")
...
OUTPUT_PDF = os.path.join(
    SCRIPT_DIR, "outputs",
    f"Hypothesis_1b_HippSubregions_QuadResponderCategory_{KIND.upper()}_Retrieval_"
...
def pretty_term(term):
    ...
    for p in parts:
        if p == "StimCondstim":
            ...
            out.append("Memory Modulation Z")
        elif p.startswith("QuadResponderGroup"):
            level = p.replace("QuadResponderGroup", "")
            out.append(f"Responder Status: {level}")
        else:
            out.append(p)
    return " x ".join(out)
...
def render_panel(pdf, title_prefix, modality, unit, band_label):
    ...
    for mod_kind, mod_label in flavors:
        ...
        if df_coef is not None and not df_coef.empty:
            ...
            if mod_kind == "QuadCategorical":
                ref_note = ("Reference categories: StimCond = nostim; "
                            "Responder Status = NonResp.")
            else:
                ref_note = "Reference category for StimCond: nostim."
            pdf.apa_table(
