##############################################################################
# Hypothesis 1b - HIPPOCAMPAL SUBREGIONS - LMM
#   Same as run_h1b_lmm_retrieval.R, but expanded scope: HPC -> {CA, DG, HPC}.
#
# Power regions: BLA, CA, DG, HPC, EC, PRC.
# Coherence pairs: BLA-{CA,DG,HPC,EC,PRC}, EC-{CA,DG,HPC},
#                  PRC-{CA,DG,HPC}, EC-PRC  (12 pairs).
# PAC pairs: BLA-{CA,DG,HPC,EC,PRC} (5 pairs).
# Bands: theta, slow_gamma. PAC: SG PAC only.
# BJH032 == BJH033 -> folded together (54 subjects).
#
# Usage: Rscript run_h1b_subregions_lmm_retrieval.R --imbalanced
##############################################################################
...
out_dir <- file.path(script_dir, "outputs",
                    paste0("h1b_subregions_lmm_", label, "_retrieval_mlm"))
...
BANDS <- list(theta = list(range=THETA, label="Theta"),
              slow_gamma = list(range=SLOW_GAMMA, label="Slow Gamma"))

POWER_REGIONS <- c("BLA", "CA", "DG", "HPC", "EC", "PRC")
COH_PAIRS <- c("BLA_CA","BLA_DG","BLA_HPC","BLA_EC","BLA_PRC",
               "CA_EC","CA_PRC","DG_EC","DG_PRC","EC_HPC","HPC_PRC","EC_PRC")
PAC_PAIRS <- c("BLA_CA","BLA_DG","BLA_HPC","BLA_EC","BLA_PRC")
...
resp$ResponderGroup <- factor(
  ifelse(resp$ResponderStatus %in% c("Strong responders","Moderate responders"), "Enhanced",
         ifelse(resp$ResponderStatus == "Anti-responders", "Impaired", "Non_responder")),
  levels = c("Non_responder","Enhanced","Impaired"))
resp$mem_mod_z <- as.numeric(scale(resp$avg_stim_dprime_diff))

cat(sprintf("Responder coverage (n=%d): %s\n", nrow(resp),
            paste(sprintf("%s=%d", levels(resp$ResponderGroup),
                          tabulate(resp$ResponderGroup)), collapse=", ")))
...
filter_balanced <- function(df, lab) {
  if (!balanced) {
    cat(sprintf("\n  [%s] No filter (imbalanced): %d patients\n",
                lab, length(unique(df$Patient)))); return(df)
  }
...
prepare_trial_df <- function(csv_path, lab) {
  ...
  merge(d, resp[, c("Patient","ResponderGroup","mem_mod_z")], by="Patient", all.x=TRUE)
}
...
run_unit <- function(sub, feature_col, modality, unit, band_label) {
  ...
  if (!is.null(fits$m_full)) {
    fr <- safe_lmer(as.formula(fmls_cont$m_full), sub)
    if (!is.null(fr)) save_coefs(fr, sprintf("%s_%s_%s_modContinuous_coefs.csv", modality, unit, band_label))
    save_model_build(fits, fmls_cont, sprintf("%s_%s_%s_modContinuous_anova.csv", modality, unit, band_label))
  }

  fmls_cat <- list(
    m0     = "feature ~ 1 + (1 | Patient)",
    m1     = "feature ~ StimCond + (1 | Patient)",
    m2     = "feature ~ StimCond + ResponderGroup + (1 | Patient)",
    m_full = "feature ~ StimCond + ResponderGroup + StimCond:ResponderGroup + (1 | Patient)")
  fits2 <- list()
  for (nm in names(fmls_cat)) fits2[[nm]] <- safe_lmer_ML(as.formula(fmls_cat[[nm]]), sub)
  if (!is.null(fits2$m_full)) {
    fr2 <- safe_lmer(as.formula(fmls_cat$m_full), sub)
    if (!is.null(fr2)) save_coefs(fr2, sprintf("%s_%s_%s_modCategorical_coefs.csv", modality, unit, band_label))
    save_model_build(fits2, fmls_cat, sprintf("%s_%s_%s_modCategorical_anova.csv", modality, unit, band_label))
  }
}

cat("\n", paste(rep("=",70),collapse=""), "\nH1B SUBREGIONS LMM POWER -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
pw <- prepare_trial_df(file.path(script_dir,"outputs","csvs","combined_retrieval_power_all_mlmr_input.csv"), "Power")
...
cat("\n", paste(rep("=",70),collapse=""), "\nH1B SUBREGIONS LMM COHERENCE -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
coh <- prepare_trial_df(file.path(script_dir,"outputs","csvs","combined_retrieval_coherence_all_mlmr_input.csv"), "Coherence")
...
cat("\n", paste(rep("=",70),collapse=""), "\nH1B SUBREGIONS LMM PAC (SG) -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
pac <- prepare_trial_df(file.path(script_dir,"outputs","csvs","combined_retrieval_pac_all_mlmr_input.csv"), "PAC")
...
cat(sprintf("\n\n===== H1B SUBREGIONS LMM RETRIEVAL (%s) COMPLETE =====\n", toupper(label)))
