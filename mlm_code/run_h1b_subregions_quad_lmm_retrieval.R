##############################################################################
# Hypothesis 1b - QUAD RESPONDER CATEGORY - LMM analyses
#   Same LMM design as run_h1b_lmm_retrieval.R, but the categorical moderator
#   uses the original four responder labels:
#     Anti-responders | Non-responders (ref) | Moderate responders | Strong responders
#
# Per region (power) or pair (coherence/PAC), per band:
#   feature ~ StimCond * QuadResponderGroup + (1 | Patient)
#
# Continuous moderator (z-scored avg_stim_dprime_diff) also fit per panel
# for direct comparison with the 3-level report.
#
# Bands: theta, slow_gamma. PAC: SG PAC only.
# BJH032 == BJH033 -> folded together (54 subjects).
#
# Usage: Rscript run_h1b_subregions_quad_lmm_retrieval.R --imbalanced
##############################################################################
...
out_dir <- file.path(script_dir, "outputs",
                    paste0("h1b_subregions_quad_lmm_", label, "_retrieval_mlm"))
...
POWER_REGIONS <- c("BLA", "CA", "DG", "HPC", "EC", "PRC")
COH_PAIRS <- c("BLA_CA","BLA_DG","BLA_HPC","BLA_EC","BLA_PRC","CA_EC","CA_PRC","DG_EC","DG_PRC","EC_HPC","HPC_PRC","EC_PRC")
PAC_PAIRS <- c("BLA_CA","BLA_DG","BLA_HPC","BLA_EC","BLA_PRC","CA_EC","CA_PRC","DG_EC","DG_PRC","EC_HPC","HPC_PRC")

# ── Responder status with 4-level QuadResponderGroup ───────────────────────
...
# Make level names R-formula-friendly (no spaces or hyphens).
...
save_coefs <- function(model, filename) {
  td <- tidy(model, effects = "fixed", conf.int = TRUE)
...
safe_lmer    <- function(formula, data) tryCatch(
  lmer(formula, data = data, REML = TRUE,  control = ctrl),
  error = function(e) { cat("    FAILED:", conditionMessage(e), "\n"); NULL })

safe_lmer_ML <- function(formula, data) tryCatch(
  lmer(formula, data = data, REML = FALSE, control = ctrl),
  error = function(e) { cat("    FAILED(ML):", conditionMessage(e), "\n"); NULL })
...
run_unit <- function(sub, feature_col, modality, unit, band_label) {
  ...
  fmls_cont <- list(
    ...
    m1     = "feature ~ StimCond + (1 | Patient)",
    m2     = "feature ~ StimCond + mem_mod_z + (1 | Patient)",
    m_full = "feature ~ StimCond + mem_mod_z + StimCond:mem_mod_z + (1 | Patient)"
  )
  fits <- list()
  for (nm in names(fmls_cont)) fits[[nm]] <- safe_lmer_ML(as.formula(fmls_cont[[nm]]), sub)
  if (!is.null(fits$m_full)) {
    full_reml <- safe_lmer(as.formula(fmls_cont$m_full), sub)
    if (!is.null(full_reml)) save_coefs(full_reml,
      sprintf("%s_%s_%s_modContinuous_coefs.csv", modality, unit, band_label))
    save_model_build(fits, fmls_cont,
      sprintf("%s_%s_%s_modContinuous_anova.csv", modality, unit, band_label))
  }

  # Quad-responder categorical moderator
  fmls_cat <- list(
    m0     = "feature ~ 1 + (1 | Patient)",
    m1     = "feature ~ StimCond + (1 | Patient)",
    m2     = "feature ~ StimCond + QuadResponderGroup + (1 | Patient)",
    m_full = "feature ~ StimCond + QuadResponderGroup + StimCond:QuadResponderGroup + (1 | Patient)"
  )
  fits2 <- list()
  for (nm in names(fmls_cat)) fits2[[nm]] <- safe_lmer_ML(as.formula(fmls_cat[[nm]]), sub)
  if (!is.null(fits2$m_full)) {
    full_reml2 <- safe_lmer(as.formula(fmls_cat$m_full), sub)
    if (!is.null(full_reml2)) save_coefs(full_reml2,
      sprintf("%s_%s_%s_modQuadCategorical_coefs.csv", modality, unit, band_label))
    save_model_build(fits2, fmls_cat,
      sprintf("%s_%s_%s_modQuadCategorical_anova.csv", modality, unit, band_label))
  }

  # ── Primary H1b tests (between-subject only, no StimCond) ────────────────
  # Direct test of H1b: do neural features differ across subjects as a
  # function of behavioral memory modulation, ignoring trial-level stim
  # condition?
  ...
  fits_pm <- list()
  for (nm in names(fmls_pri_mem)) fits_pm[[nm]] <- safe_lmer_ML(as.formula(fmls_pri_mem[[nm]]), sub)
  if (!is.null(fits_pm$m1)) {
    pri_mem_reml <- safe_lmer(as.formula(fmls_pri_mem$m1), sub)
    if (!is.null(pri_mem_reml)) save_coefs(pri_mem_reml,
      sprintf("%s_%s_%s_primaryMemMod_coefs.csv", modality, unit, band_label))
    save_model_build(fits_pm, fmls_pri_mem,
      sprintf("%s_%s_%s_primaryMemMod_anova.csv", modality, unit, band_label))
  }
  ...
  fits_pg <- list()
  for (nm in names(fmls_pri_grp)) fits_pg[[nm]] <- safe_lmer_ML(as.formula(fmls_pri_grp[[nm]]), sub)
  if (!is.null(fits_pg$m1)) {
    pri_grp_reml <- safe_lmer(as.formula(fmls_pri_grp$m1), sub)
    if (!is.null(pri_grp_reml)) save_coefs(pri_grp_reml,
      sprintf("%s_%s_%s_primaryRespGroup_coefs.csv", modality, unit, band_label))
    save_model_build(fits_pg, fmls_pri_grp,
      sprintf("%s_%s_%s_primaryRespGroup_anova.csv", modality, unit, band_label))
  }
}

##############################################################################
cat("\n", paste(rep("=",70),collapse=""), "\nH1B QUAD LMM POWER -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
...
cat("\n", paste(rep("=",70),collapse=""), "\nH1B QUAD LMM COHERENCE -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
...
cat("\n", paste(rep("=",70),collapse=""), "\nH1B QUAD LMM PAC (SG) -", toupper(label), "\n",
    paste(rep("=",70),collapse=""), "\n", sep="")
...
cat(sprintf("\n\n===== H1B QUAD LMM RETRIEVAL (%s) COMPLETE =====\n", toupper(label)))
