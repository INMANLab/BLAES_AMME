#!/usr/bin/env python
"""Paired within-subject t-tests for retrieval power.

This report tests stim vs no-stim retrieval power separately for remembered and
forgotten trials, using only Theta (4-8 Hz) and Slow gamma (35-50 Hz) band
averages from the baseline-corrected retrieval spectra.
"""
...
OUTPUT_DIR = SCRIPT_DIR / "outputs" / "parametric retrieval test" / "stim vs no stim" / "t-tests" / "power"
...
ROI_ORDER = ["BLA", "CA", "DG", "HPC", "EC", "PHG", "PRC", ALLHPC, MTL]
...
def paired_band_stats(data):
    ...
    if freqs is None:
        raise RuntimeError("Retrieval diff frequencies were not available.")
...
def make_title_page():
    ...
    y = 0.97
    ax.text(0, y, "Retrieval Power Paired T-Tests", fontsize=18, fontweight="bold", va="top")
    y -= 0.06
    ax.text(0, y, "Stim vs No Stim, remembered and forgotten trials separately", fontsize=12, va="top")
    y -= 0.05

    paragraphs = [
        "Tests are paired within-subject t-tests comparing stim and no-stim retrieval power.",
        "Each value is the mean baseline-corrected power within the selected band for a given region and patient.",
...
def main():
    ...
    if stats_df.empty:
        raise RuntimeError("No paired retrieval power statistics could be computed.")
    ...
    with PdfPages(PDF_PATH) as pdf:
        ...
        for memory_label in ["Remembered", "Forgotten"]:
            ...
            for band_label in ["Theta", "Slow gamma"]:
                ...
                if band_df.empty:
                    continue
                pdf.savefig(make_table_page(f"{memory_label} Trials | {band_label}", band_df))
                plt.close("all")

        pdf.savefig(make_summary_page(stats_df))
        plt.close("all")

    print(f"Wrote paired retrieval power tests to: {PDF_PATH}")
