...
excluding Memory_Z to preserve sample size.
...
OUTPUT_PDF = OUTPUT_DIR / "between_subjects_variability_report_no_memoryz.pdf"
...
def add_data_page(pdf: PdfPages, missingness: pd.DataFrame, sex_descriptives: pd.DataFrame, df: pd.DataFrame) -> None:
    ...
    draw_apa_table(
        ...
        note=(
            "Memory_Z is shown for context but was excluded from the modeled predictor set in this report to avoid reducing "
            "the sample from 54 to 31 patients. IED_freq is available for 44/54 patients."
        ),
        font_size=8.8,
        bbox=[0, 0.08, 1, 0.84],
    ...
    fig.text(
        ...
        wrap(
            ...
            "Memory_Z was excluded by design because modeling it would have restricted the analysis to a narrower 31-patient subset.",
            110,
        ),
        fontsize=9.5,
        family="DejaVu Serif",
...
def add_conclusion_page(pdf: PdfPages, primary_result: LinearModelResult, results: list[LinearModelResult]) -> None:
    ...
    paragraphs = [
        ...
        "Memory_Z was intentionally excluded from this report because it would have cut the modeled sample down to 31 patients and narrowed the contributing study cohorts.",
