...
excluding Memory_Z to preserve sample size, using the balanced memory-trials subset.
...
All outputs are written under outputs/testing_between_subjects_variability_MLM_no_memoryz/.
"""
...
OUTPUT_PDF = OUTPUT_DIR / "between_subjects_variability_report_balanced_trials_no_memoryz.pdf"
FILE_PREFIX = "balanced_trials_no_memoryz_"
...
def load_behavior() -> pd.DataFrame:
    df = load_behavior_csv().copy()
    counts_df = load_trial_counts()
    _, df = apply_balanced_filter(df, counts_df)
...
def add_title_page(
    pdf: PdfPages,
    sex_test: dict,
    primary_result: LinearModelResult,
    total_rows: int,
    total_patients: int,
) -> None:
    fig, ax = start_page()
    ...
    ax.text(
        0.5,
        0.915,
        "Patient-level linear models using the balanced behavioral first-session summary file",
        ha="center",
        va="top",
        fontsize=11,
        family="DejaVu Serif",
    ...
    draw_wrapped_text(
        ...
        0.08,
        0.76,
        f"Critical modeling point: the balanced file used here has one row per patient ({total_rows} rows, {total_patients} unique patients). "
    ...
    bullets = [
        ...
        "This version of the report intentionally omits Memory_Z so the modeled sample stays as large as possible.",
        "IED_freq still did not improve model fit in the smaller subset where it was available.",
    ]
    ax.text(
        0.08,
        0.62,
...
def add_data_page(pdf: PdfPages, missingness: pd.DataFrame, sex_descriptives: pd.DataFrame, df: pd.DataFrame) -> None:
    ...
    draw_apa_table(
        ...
        "Table 1\nVariable availability in the balanced behavioral summary file",
        note=(
            "Memory_Z is shown for context but was excluded from the modeled predictor set in this report to preserve "
            f"the balanced-trials sample. IED_freq is available for {int(df['IED_freq'].notna().sum())}/{len(df)} balanced-trials patients."
        ),
        font_size=8.8,
        bbox=[0, 0.08, 1, 0.84],
    ...
    fig.text(
        ...
        0.11,
        wrap(
            "This report keeps the full-sample adjusted model and the IED-frequency sensitivity model only. "
            f"Memory_Z was excluded by design because modeling it would have restricted the analysis to a narrower {int(df['Memory_Z'].notna().sum())}-patient subset.",
            110,
        ),
        fontsize=9.5,
        family="DejaVu Serif",
...
def add_sensitivity_pages(pdf: PdfPages, results: list[LinearModelResult]) -> None:
    ...
    ax_title.text(
        ...
        0.35,
        wrap(
            "The sensitivity analysis here is limited to IED_freq so the report stays focused on models that retain most of the cohort. "
            "Memory_Z was excluded from this version by design because of its heavier missingness.",
            112,
        ),
        fontsize=10.5,
        family="DejaVu Serif",
    ...
    draw_apa_table(
        ...
        note="The IED sensitivity model is the only reduced-sample model retained in this report.",
        font_size=8.2,
        bbox=[0, 0.06, 1, 0.90],
...
def add_conclusion_page(
    ...
    paragraphs = [
        ...
        "and adding it did not improve explanatory power.",
            f"Memory_Z was intentionally excluded from this report because it would have cut the modeled sample down to {int(df['Memory_Z'].notna().sum())} patients and narrowed the contributing study cohorts.",
...
def main() -> None:
    ...
    with PdfPages(OUTPUT_PDF) as pdf:
        add_title_page(pdf, sex_test, results[0], len(df), df["Patient"].nunique())
        ...
        add_conclusion_page(pdf, results[0], results, df)
