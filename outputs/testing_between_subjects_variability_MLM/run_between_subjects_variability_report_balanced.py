...
using the balanced memory-trials subset.
...
def add_data_page(pdf: PdfPages, missingness: pd.DataFrame, sex_descriptives: pd.DataFrame, df: pd.DataFrame) -> None:
    ...
    draw_apa_table(
        ...
        note=(
            f"Memory_Z is available for {int(df['Memory_Z'].notna().sum())}/{len(df)} balanced-trials patients and "
            f"IED_freq is available for {int(df['IED_freq'].notna().sum())}/{len(df)}."
        ),
        font_size=8.8,
        bbox=[0, 0.08, 1, 0.84],
...
def add_sensitivity_pages(pdf: PdfPages, results: list[LinearModelResult]) -> None:
    ...
    ax_title.text(
        ...
        0.35,
        wrap(
        "The sensitivity models show what happens when IED_freq and Memory_Z are brought into the balanced-trials analysis. "
