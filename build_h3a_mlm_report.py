...
def main():
    ...
    pdf.apa_table(
        ...
        note=f'GLMM with logit link and random intercept for patient (N = {n_trials} trials, '
             ...
             f'remembering when IED is present. *p < .05. **p < .01. ***p < .001.'
    )
    ...
    # ICC
    # random intercept variance from model summary text
    # We read it from the tidy output or compute from known values
    # For the report, note the random intercept variance
    pdf.body_text(
