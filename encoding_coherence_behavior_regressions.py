#!/usr/bin/env python
"""
Patient-level regressions between encoding coherence baseline-corrected
connectivity and memory modulation (avg_stim_dprime_diff).

Baseline-corrected functional connectivity measure:
1. Within each patient and region pair, average baseline-corrected encoding
   coherence separately for remembered and forgotten trials.
2. Average the available stim and nostim spectra within each memory condition
   to obtain one remembered and one forgotten spectrum per patient and ROI.
3. Collapse each memory-specific spectrum within a target band to obtain the
   regression x-axis values.
4. For composite ROIs, first average the patient's mean spectra across source
    ...
    for roi in source_rois:
        for band_name in BAND_SPECS:
            ...
            plot_memory_panel_histogram(
                ...
                "Baseline-Corrected Functional Connectivity",
                "Encoding Coherence Distribution",
            ...
            for memory_name in MEMORY_ORDER:
                ...
                if memory_df.empty:
                    continue
                stats_row = fit_regression(memory_df, "baseline_fc", TARGET_COLUMN, "baseline_fc")
            ...
            plot_memory_panel_regression(
                ...
                BAND_LABELS[band_name],
                "Baseline-Corrected Functional Connectivity",
                "Encoding Coherence vs Memory Modulation",
                detail_dir / f"{roi}_{band_name}_baseline_fc_vs_memory_modulation.png",
                LOGIC_NOTE,
                point_color="#1f4e79",
                line_color="#b22222",
                undefined_label="baseline_fc",
            )
