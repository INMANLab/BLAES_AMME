...
def figure2_ied_features_memory(dm):
    ...
    # IED Rate
    r, p = compute_effect_size_memory(dm['Rate(within trials)'], dm['memory_binary'])
    features_results.append({'Feature': 'IED Rate\n(IEDs/trial)', 'beta': r, 'p': p})
    stats_rows.append({'Feature': 'IED Rate', 'r_or_beta': r, 'p_value': p, 'test': 'point-biserial'})
    print(f'  IED Rate: r={r:.4f}, p={p:.4g}')
    ...
    if len(left_rem) > 5 and len(right_rem) > 5:
        ...
        features_results.append({'Feature': 'IED Hemisphere\nRight -\nLeft (ref)', 'beta': diff, 'p': p})
        stats_rows.append({'Feature': 'Hemisphere (R-L)', 'r_or_beta': diff, 'p_value': p, 'test': 'Mann-Whitney'})
        print(f'  Hemisphere (R-L): diff={diff:.4f}, p={p:.4g}')
    ...
    if len(mtl_rem) > 5 and len(nonmtl_rem) > 5:
        ...
        features_results.append({'Feature': 'Region\nnon-MTL -\nMTL (ref)', 'beta': diff, 'p': p})
        stats_rows.append({'Feature': 'Region (nonMTL-MTL)', 'r_or_beta': diff, 'p_value': p, 'test': 'Mann-Whitney'})
        print(f'  Region (nonMTL-MTL): diff={diff:.4f}, p={p:.4g}')
    ...
    if len(gray_rem) > 5 and len(white_rem) > 5:
        diff = gray_rem.mean() - white_rem.mean()
        _, p = mannwhitneyu(gray_rem, white_rem, alternative='two-sided')
        features_results.append({'Feature': 'White Matter\nPropagation\nYes - No (ref)', 'beta': -diff, 'p': p})
        stats_rows.append({'Feature': 'White Matter (W-G)', 'r_or_beta': -diff, 'p_value': p, 'test': 'Mann-Whitney'})
        print(f'  White Matter (W-G): diff={-diff:.4f}, p={p:.4g}')

    # Channel Spread
    r, p = compute_effect_size_memory(dm['ChannelSpread'], dm['memory_binary'])
    features_results.append({'Feature': 'Channel\nSpread', 'beta': r, 'p': p})
    stats_rows.append({'Feature': 'Channel Spread', 'r_or_beta': r, 'p_value': p, 'test': 'point-biserial'})
    print(f'  Channel Spread: r={r:.4f}, p={p:.4g}')

    # Region Spread
    r, p = compute_effect_size_memory(dm['RegionSpread'], dm['memory_binary'])
    features_results.append({'Feature': 'Region\nSpread', 'beta': r, 'p': p})
    stats_rows.append({'Feature': 'Region Spread', 'r_or_beta': r, 'p_value': p, 'test': 'point-biserial'})
    print(f'  Region Spread: r={r:.4f}, p={p:.4g}')

    # During Image/ITI timing
    if 'DuringImg' in dm.columns:
        img_data = dm[dm['DuringImg'].isin(['Y', 'N'])].copy()
        during_rem = img_data[img_data['DuringImg'] == 'Y']['memory_binary']
        notduring_rem = img_data[img_data['DuringImg'] == 'N']['memory_binary']
        if len(during_rem) > 5 and len(notduring_rem) > 5:
            diff = during_rem.mean() - notduring_rem.mean()
            _, p = mannwhitneyu(during_rem, notduring_rem, alternative='two-sided')
            features_results.append({'Feature': 'During Image/ITI\nYes -\nNo (ref)', 'beta': diff, 'p': p})
            stats_rows.append({'Feature': 'During Image/ITI (Y-N)', 'r_or_beta': diff, 'p_value': p, 'test': 'Mann-Whitney'})
            print(f'  During Image/ITI: diff={diff:.4f}, p={p:.4g}')
    ...
    ax_a.set_ylabel('Effect Size (r or prop. difference)', fontsize=12)
    ax_a.set_title(f'(A) {PHASE_LABEL}: IED Features Associated with Memory', fontsize=14, fontweight='bold')
    for i, row in fr.iterrows():
        star = _sig_star(row['p'])
        if star:
            y = row['beta']
            offset = 0.005 if y >= 0 else -0.01
            ax_a.text(i, y + offset, star, ha='center', va='bottom' if y >= 0 else 'top',
