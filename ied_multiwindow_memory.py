...
def plot_dose_response(trial_level, phase_label, filename):
    ...
    trial_level['remembered_binary'] = (trial_level['MemoryOutcome'] == 'remembered').astype(int)
    rho, p_spearman = spearmanr(trial_level['n_windows'], trial_level['remembered_binary'])
    ...
    ax.set_title(f'{phase_label}: IED "Dose-Response"\nMore Windows \u2192 Worse Memory?',
...
def plot_combinations(trial_level, phase_label, filename):
    ...
    for bar, row in zip(bars, combo_df.itertuples()):
