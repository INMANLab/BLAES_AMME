...
def compute_effect_size_memory(feature_series, memory_series):
    """Compute point-biserial r between a continuous feature and binary memory."""
...
def figure2_ied_features_memory(dm):
    ...
    def add_binary_feature(column, yes_value, no_value, plot_label, stats_label):
        sub = dm[dm[column].isin([yes_value, no_value])].copy()
        yes_rem = sub[sub[column] == yes_value]['memory_binary']
        no_rem = sub[sub[column] == no_value]['memory_binary']
        if len(yes_rem) <= 5 or len(no_rem) <= 5:
            return

        diff = yes_rem.mean() - no_rem.mean()
        _, p = mannwhitneyu(yes_rem, no_rem, alternative='two-sided')
        features_results.append({'Feature': plot_label, 'beta': diff, 'p': p})
        stats_rows.append({
            'Feature': stats_label,
            'r_or_beta': diff,
            'p_value': p,
        ...
        print(f'  {stats_label}: diff={diff:.4f}, p={p:.4g}')
